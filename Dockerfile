...
COPY . .
ENV NEXT_TELEMETRY_DISABLED=1 WHATSNEXT_STANDALONE=1
RUN npm run build
...
ENV NODE_ENV=production DATABASE_PATH=/app/data/whatsnext.sqlite
...
WORKDIR /app
ENV NODE_ENV=production NEXT_TELEMETRY_DISABLED=1 HOSTNAME=0.0.0.0 PORT=3000 DATABASE_PATH=/app/data/whatsnext.sqlite
