...
- Classes and the personal timetable are shared only through an accepted friendship. Requests go to schoolmates only; a crossing request becomes a friendship. Removing a friend, blocking, or support removal revokes access at once because every server read re-checks the friendship. The list of friends' class names per period in the workspace summary (used to show who shares each class) is held in memory only and never written to the device's offline store, so an app that is already open keeps showing a former friend there only until its next successful sync, and an offline start shows no friends at all. At most 30 unanswered outgoing requests.
- Blocking hides both people from each other and ends any friendship. In the public Global chat room the block is one way: the blocker stops seeing the blocked member's messages (docs/CHAT.md §11). Reports (private, at most 10 open per reporter) reach the support inbox, where the owner can dismiss them or remove the member from the school; removal drops verification and friendships, resolves open reports, and prevents rejoining.
- Voting exists only for member-locked schools (10 or more members). Verified members propose one open change each (five open per school) and vote for or against; the proposer counts as a vote for. Only votes from people who are still verified members count while a proposal is open (a closed proposal keeps showing the counts that decided it), and a member who leaves, is removed or loses verification has their open proposal withdrawn. A proposal passes at max(3, 20% of verified members, capped at 25) votes for with more for than against, and is rejected once votes against reach that threshold and at least equal the votes for, so a tie at or above the threshold (possible only after the threshold falls because verified members left) rejects. Passing applies a new unapproved revision that members review as usual. On support-locked schools a passed proposal waits for the owner, who publishes it keeping approval and lock, or declines it. A proposal is superseded when the school revision changes underneath it.
...
- There are no class groups or school rooms. Class membership is self-declared, so a class room would let anyone reach minors. Friend groups of up to 20, created and administered by one student from their accepted friends, exist since 2026-09-25 ([CHAT.md §12](docs/CHAT.md#12-friend-groups-migration-15)); students see the reason for a messaging pause or a removal and can appeal it ([§14](docs/CHAT.md#14-sanctions-the-student-can-see-and-appeal-migration-15)). Amended 2026-09-24 ([CHAT.md §11](docs/CHAT.md#11-global-chat-migration-7)): one public Global chat room across schools, which any member with names entered can read and post in without a friendship. The owner moderates it directly (edit or remove with a visible reason, audited), it has no block or report button of its own, but a block made in People or a one-to-one chat hides the blocked member's room messages from the blocker (one way), members can mute it, and pauses, send limits, pushes and retention follow the one-to-one rules.
...
- Messages are text only, up to 1,000 characters after normalization, with a small markdown subset (bold, italic, strikethrough, code, quotes, lists) rendered without HTML. Only `https://` links are clickable, the full URL is always shown, and the server never fetches a URL. Since 2026-09-25 ([CHAT.md §13](docs/CHAT.md#13-profile-pictures-formatting-receipts-and-typing-migration-15)) one-to-one chats and friend groups show delivery and read receipts and a typing row (the owner's call, reversing D10); there is still no presence or "last seen". Members have profile pictures (uploaded, or their Google photo, or an initial).
...
- A report freezes up to 30 messages from that one chat (or 15 before and 14 after a reported message) with a category. `danger` reports sort first and show the reporter 911 and 988. The owner sees one-to-one chat text only by pressing Show messages on a report, and every view is written to the audit log. No admin procedure reads live one-to-one messages. The Global chat room is public, so the owner reads it like any member.
...
- Retention: messages are deleted after 180 days, the text of deleted messages is erased after 30 days, report snapshots are cleared 180 days after the report is resolved, and threads idle for 180 days are deleted. Global chat messages follow the same 180 and 30 day rules.
- Exit gate met: messaging scope and abuse-handling behavior are specified in docs/CHAT.md and verified by `src/server/chat.test.ts` (scope, idempotent sends, body rules without echo, rate limits, revocation, closed rows, unread counts, cursors, reports, the admin procedure allowlist, pauses, account binding, retention, reserved names, migration), `src/server/global-chat.test.ts` for the Global chat room, the `deliverChat` cases in `src/server/notifications.test.ts`, the worker order in `src/server/jobs.test.ts`, and the seven browser scenarios in `tests/e2e/chat.spec.ts`.
