...
- Messages are text only, up to 1,000 characters after normalization, with a small markdown subset (bold, italic, strikethrough, code, quotes, lists) rendered without HTML. Only `https://` links are clickable, the full URL is always shown, and the server never fetches a URL. Since 2026-09-25 ([CHAT.md §13](docs/CHAT.md#13-profile-pictures-formatting-receipts-and-typing-migration-17)) one-to-one chats and friend groups show delivery and read receipts and a typing row (the owner's call, reversing D10); there is still no presence or "last seen". Members have profile pictures (uploaded, or their Google photo, or an initial).
