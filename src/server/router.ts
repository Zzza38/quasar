...
const calendarAccount = authenticated.input(z.object({ accountId: z.uuid() })).use(({ ctx, input, next }) => {
  if (input.accountId !== ctx.userId) throw new TRPCError({ code: 'UNAUTHORIZED', message: 'The signed-in account changed. Sign in to the original account to manage its calendars.' });
  return next({ ctx });
});
export const appRouter = t.router({
  ...
  calendar: t.router({
    ...
    subscribe: calendarAccount.input(subscribeSchema).mutation(({ ctx, input }) => new CalendarService(ctx.service.db).subscribe(ctx.userId, input)),
    refresh: calendarAccount.input(z.object({ id: z.uuid() })).mutation(({ ctx, input }) => new CalendarService(ctx.service.db).refresh(ctx.userId, input.id)),
    setEnabled: calendarAccount.input(z.object({ id: z.uuid(), enabled: z.boolean() })).mutation(({ ctx, input }) => new CalendarService(ctx.service.db).setEnabled(ctx.userId, input.id, input.enabled)),
    remove: calendarAccount.input(z.object({ id: z.uuid() })).mutation(({ ctx, input }) => new CalendarService(ctx.service.db).remove(ctx.userId, input.id)),
    resolve: calendarAccount.input(z.object({ entityId: z.string().min(1).max(100), expectedVersion: z.number().int().positive(), choice: z.enum(['local', 'source']) })).mutation(({ ctx, input }) => new CalendarService(ctx.service.db).resolve(ctx.userId, input.entityId, input.expectedVersion, input.choice)),
  }),
  school: t.router({
