...
const row = (name: string, extra: Partial<Parameters<typeof applyScan>[1][number]> = {}) => ({ name, days: [], include: true, ...extra });
...
describe('applying a confirmed scan', () => {
  it('creates classes with stable IDs and places them on periods', () => {
    const next = applyScan(emptyPersonalSchedule(), [
      row('Algebra II', { periodId: 'P1', teacher: 'Ms. Ortiz', room: '204' }),
      row('Chemistry', { periodId: 'P2', directoryId: DIR }),
      row('Unplaced elective'),
      row('Skipped', { include: false, periodId: 'P3' }),
      row('   ', { periodId: 'P4' }),
    ]);
    ...
    expect(next.assignments).toEqual({ P1: 'Algebra-II', P2: DIR });
  });
  ...
  it('reuses classes the student already has instead of duplicating them', () => {
    ...
    const next = applyScan(personal, [row('Algebra II', { periodId: 'P1' }), row('Chemistry', { periodId: 'P2', directoryId: DIR }), row('Algebra II', { periodId: 'P3' })]);
