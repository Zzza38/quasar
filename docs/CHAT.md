...
*Amended 2026-09-25 by §12:* friend groups exist: a named group of up to `CHAT.groupMaxMembers` (20) people that one student creates from their accepted friends and administers. Class groups and school rooms are still out.
...
*Reversed 2026-09-25 by the owner (§13):* one-to-one chats and friend groups now show "Delivered", "Read" (or "Read by n") under the sender's newest message and a "typing…" row. The safety reasoning above still holds; the owner weighed it against students expecting these from every other messaging app. The Global chat room has neither. There is still no "online now" or "last seen". A per-account switch to turn receipts off is the natural next step if the concern turns out to be real.
...
- Group chats beyond the single global room in §11 and friend groups in §12: class groups, school-wide rooms. Messages or message requests from people who aren't friends.
- Images, files, voice, stickers, GIFs, reactions, replies, forwarding, editing (delete and resend instead), pinning, search, export. (Markdown quotes exist as text formatting, §13; profile pictures exist, §13, but pictures inside messages do not.)
- Link previews, link reputation checks, and any server fetch of message content. Clickable `http://` and other non-https schemes.
- Online presence and "last seen". (Read receipts and typing signals were added in §13; D10 records why they were first left out.)
...
**Where it lives.** The chat list's **Groups** section holds the Global chat row and then every group the student is in, most recently active first, each with stacked member pictures, "Name: text" preview, unread pill and mute icon, and a **New group** button (name plus a checklist of friends). A group opens at `/messages?group=<id>` (immersive on phones). Its header shows the members' pictures, the name (a button that opens the members sheet), a member count and "You're the admin" for the admin; its menu offers Members (and, for the admin, adding people and Rename group), Mute and Leave group. Bubbles carry the sender's name and picture on the first bubble of a run, as in the room.

**Rules.** A group takes `CHAT.groupsPerDay` (5) creations per student per rolling day and shares the per-minute and per-day send budget with every other chat (`countSentSince` counts three tables). Pauses apply: a paused student can read but not send, and cannot create a group or add people. A block made anywhere applies one way, exactly as in the room: the blocker no longer sees the blocked member's group messages (thread, list preview, unread count, pushes), and the blocked member's view does not change. The admin can remove anyone's message ("Removed by the group admin", audited as `group.remove`); a sender can delete their own. Support removal (`removeFromSchool`) ends every group membership of the removed student (`leaveAllGroups`).

**Reports.** A member reports one message (the §3.1 anchored window: 15 before, the message, 14 after) against its sender, with the optional block. The frozen snapshot holds every member's messages in that window, each tagged with `senderId`, so `admin.showEvidence` names each sender; `admin.redactMessage` hides a group message the same way. `reports.group_id` marks a group report (no foreign key, like `thread_id`); `admin.reports` counts it as a chat report. One open report per (reporter, group).
...
**Profile pictures.** Every member row (`avatarUrl` in `src/server/avatars.ts`) carries `avatar: string | null`: the picture the student uploaded (`/api/avatars/<id>?v=<version>`, a route handler that requires a signed-in session and lets the browser keep the versioned URL for a year), else their Google profile picture (saved at sign-in from the OpenID `picture` claim, only when it is an https URL on `googleusercontent.com`, which the CSP `img-src` allows), else null for a gradient initial. The account sheet's **Profile photo** controls upload a photo (cropped square and scaled to 256 px in the browser, `encodeAvatar` in `avatar-settings.tsx`, at most `AVATAR_MAX_BYTES` (200 KB) of JPEG, PNG or WebP whose bytes are sniffed on the server), switch to the Google photo, or show only the initial (`avatar.upload`, `avatar.clear({ source })`). Pictures show in the chat list, thread headers, group bubbles, the members sheet, People, the profile sheet and the shell. `CONTEXT_VERSION` is 3.

**Formatting.** `src/domain/markdown.ts` parses a body into blocks (paragraph, fenced code, `>` quote, `-`/`1.` list) and inlines (`**bold**`, `_italic_` or `*italic*`, `~~strike~~`, `` `code` ``, https links). The bubble renders that tree as React nodes; text nodes are censored at display time as before. A run of three or more markers is plain text, so the asterisks a censored slur leaves behind never turn into formatting, and `_` inside a word never italicises snake_case. List previews and "Add as task" use `plainText`. The composer's **Formatting** button shows a toolbar (Bold, Italic, Strikethrough, Code) that wraps the selection, with Ctrl/Cmd+B, I, Shift+X and E as shortcuts.

**Delivery and read receipts.** `chat_members.delivered_seq` (and the group equivalent) records the newest message a member's device has fetched: `chat.thread`, `chat.inbox` and the group equivalents update it after their read transaction, only when the response carries something newer, so an idle poll takes no write lock. `last_read_seq` is unchanged (§6). Every thread page carries `receipts: { id, displayName, avatar, deliveredSeq, readSeq, typing }[]` for the other members (empty in the room), read from the base of every poll, so no revision bump is needed. The client shows a caption under the viewer's newest own message only: "Sent", "Delivered" or "Read" in a one-to-one chat; "Sent", "Delivered to n" / "Delivered", "Read by n" / "Read by everyone" in a group.
...
- `ChatPause` is now `{ until, reason, appealed }`; the pause banner in the chat list and the composer's replacement show the reason and an **Appeal** button (or "Appeal sent" once one is open). The pause dialog on `/admin` labels the reason "Reason (shown to the student)".
- The workspace carries `sanctions: { bans: { schoolId, schoolName, reason, createdAt, appealed }[], pause }`. A removed student lands on the school step, which shows a callout per removal ("Support removed you from {school}", the reason, Appeal) and says they can still join another school; `school.join` on a banned school points there.
- `support.appeal({ kind: 'pause' | 'ban', schoolId?, message })` inserts a `support_requests` row of `kind` `appeal:pause` or `appeal:ban` (a new column; existing requests and feedback have `''`), one open appeal per sanction (`CONFLICT` "You already sent an appeal. Support will review it."), under the existing five-open-requests cap. It appears in the owner's "Correction requests and appeals" inbox with an "Appeal: …" chip, the student's name and buttons **Lift pause** / **Lift removal** (which resolve the appeal) or **Keep as is** (which only resolves it).
- `admin.bans` and `admin.liftBan({ userId, schoolId })` back a new **Removed members** section on `/admin`; lifting a removal deletes the `school_bans` row, resolves the open appeal and writes `member.restore` to `audit_log`. `admin.liftChatPause` now also resolves an open pause appeal.
- Tests: `src/server/sanctions.test.ts`, and the appeal steps of the harassment scenario in `tests/e2e/chat.spec.ts`.
