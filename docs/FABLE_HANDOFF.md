...
| Root and routing | [tracker.tsx](../src/components/tracker.tsx), [app-state.ts](../src/components/app-state.ts) | Path routes (`/`, `/tasks`, …) are rendered by the server for the signed-in account; the service worker's offline copy of `/` opens any of them. Builds the `AppState` passed to every view. |
...
| Messages | [views/messages.tsx](../src/components/views/messages.tsx) | `MessagesView` handles `/messages` (chat list, and on desktop the two-pane layout) and `/messages?with=<userId>` (thread, composer, report, delete and block dialogs, closed and paused states). Every call goes straight to `api.chat.*` with `accountId`; see [CHAT.md](CHAT.md). |
| Chat polling and sends | [use-chat.ts](../src/components/use-chat.ts) | `useChatInbox`, `useChatThread` (visibility- and online-aware polls with backoff, the revision cursor, the ordered send queue with one automatic retry), `useChatViewport` (`--chat-h` from `visualViewport` for the phone thread), `clearChatMemory`. Drafts and pending sends live in a module map keyed by `${accountId}:${userId}` and are cleared when the account changes. |
...
5. **Device conflicts:** field comparisons with clear local/remote choices. The local choice keeps the latest optimistic entity, including later queued edits, at the competing paths and the other side's independent edits everywhere else (the whole local version when that mix would be invalid); the comparison shows that latest value. For an imported task whose calendar changed a competing field (title, notes or due date/time) the other side is labelled "Calendar"; a calendar refresh that only restamped the event's source details does not count, so the other side stays "Other device". A change the server refused outright shows here too, with the server's reason, **Try again** and **Discard my changes to this task** (**Discard my schedule changes** for the personal schedule), which drops every change this device has not saved to that item.

Generate an ID once when creating a period, class, cycle day or slot (`slugId`, `randomId` in `format.ts`). Keep it when renaming, reordering or editing. Slots are chronological and cannot overlap or cross midnight. Several slots can reference the same period, so a class can appear before and after lunch. Lunch waves can use distinct lunch period IDs. Removing a class clears its period assignments in the same personal save, and drops the private `class-<id>` period ClassAssignmentGrid made for it along with that period's slots (`removeClass` in `personal-timetable.ts`); school periods and other private periods keep their times. Existing task data remains intact.
...
| `taskSchema`, `Task` in [task.ts](../src/domain/task.ts); `sortByDue` in [app-state.ts](../src/components/app-state.ts) | Task form validation; sorting tasks by due date and time |
...
Name entry, school selection/creation, shared corrections, support requests, review acknowledgment and admin operations are online operations through `api`. Chat is outside the offline queue contract: messages, drafts and unsent messages stay in memory for the open tab and never enter the store, the queue or localStorage. The only chat action that goes through the queue is **Add as task**, because it creates an ordinary task with `state.saveTask`. Shared saves include `expectedVersion`; a stale revision must produce a reviewable error while retaining the form draft.
...
- The workspace context includes `community.classmates` (accepted friends' names and class names per period), but it is held in memory only: `persistedContext` in `src/components/app-state.ts` empties it before the context is written to IndexedDB, so an offline start shows no "With …" tags until the first successful sync, and an already open app drops a revoked friend on its next successful sync, as documented in [ARCHITECTURE.md](ARCHITECTURE.md) phase 3. Do not write any friend data to the device store.
...
Phase 1 was kept to personal schedules and tasks; automatic iCalendar feeds, subtasks, recurring tasks, priorities, notifications and reminders followed in phase 2, and member browsing, verification workflows, friends, voting and chat in phases 3 and 4 (see the README). Google email verification is authentication and does not establish school verification.
