...
## Owner access and the audit log

The Support page (`/admin`) can read and change any account (docs/ARCHITECTURE.md, "Owner console and its security"). It needs a Google sign-in from the last two hours; after that it asks you to sign in again. Every owner action lands in `audit_log` with the request's address and browser, and a copy is printed to the server log (`journalctl --user -u quasar | grep '^audit '` on the pilot host), which the database cannot rewrite. `audit_log` is append-only (triggers from migration 16): a restore replaces the whole file, and nothing should ever edit rows. If the Audit log tab shows an owner action you did not take, open your own record in People and choose **Sign out everywhere**: every session of the owner account ends, this one included, and you sign in again with Google. Then check your Google account's security and review the log.
