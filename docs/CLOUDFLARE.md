...
- Turn on SSL/TLS > Edge Certificates > Always Use HTTPS for the zone (as of 2026-09-25 it is off: `http://quasar.ziona.dev/` answers 200 with the app; the app now redirects proxied plain-HTTP requests itself, but the edge switch is the proper fix and spares the origin the request). The tunnel otherwise answers `http://quasar.ziona.dev` with the app over cleartext; check that `curl -sI http://quasar.ziona.dev/` returns a 301 to `https://`. The app's own `Strict-Transport-Security` header (set in `next.config.ts` with its Content-Security-Policy and Permissions-Policy) only takes effect after a browser has loaded the HTTPS origin once.
